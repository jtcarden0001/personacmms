...
func (pg *PostgresStore) CreateTimeTrigger(tt tp.TimeTrigger) (tp.TimeTrigger, error) {
	query := fmt.Sprintf(`
			INSERT INTO %s (id, quantity, timeunit_id, task_id) 
			VALUES ($1, $2, $3, $4)`,
		timeTriggerTableName)
...
func (pg *PostgresStore) DeleteTimeTriggerFromTask(taskId uuid.UUID, ttId uuid.UUID) error {
	query := fmt.Sprintf(`
			DELETE FROM %s 
			WHERE task_id=$1 AND id=$2`,
		timeTriggerTableName)

	result, err := pg.db.Exec(query, taskId, ttId)
	if err != nil {
		return handleDbError(err, "time-trigger")
	}
...
func (pg *PostgresStore) GetTimeTrigger(id uuid.UUID) (tp.TimeTrigger, error) {
	query := fmt.Sprintf(`
			SELECT id, quantity, timeunit_id, task_id 
			FROM %s 
			WHERE id=$1`,
		timeTriggerTableName)
...
func (pg *PostgresStore) ListTimeTriggersByTask(taskId uuid.UUID) ([]tp.TimeTrigger, error) {
	query := fmt.Sprintf(`
			SELECT id, quantity, timeunit_id, task_id
			FROM %s
			WHERE task_id = $1`,
...
func (pg *PostgresStore) UpdateTimeTrigger(tt tp.TimeTrigger) (tp.TimeTrigger, error) {
	query := fmt.Sprintf(`
			UPDATE %s 
			SET quantity=$1, timeunit_id=$2, task_id=$3 
			WHERE id=$4`,
		timeTriggerTableName)
