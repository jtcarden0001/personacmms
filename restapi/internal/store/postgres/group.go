...
func (pg *PostgresStore) CreateGroup(g tp.Group) (tp.Group, error) {
	query := fmt.Sprintf(`INSERT INTO %s (id, title) VALUES ($1, $2)`, groupTableName)
...
func (pg *PostgresStore) DeleteGroup(id uuid.UUID) error {
	query := fmt.Sprintf(`DELETE FROM %s WHERE id = $1`, groupTableName)
...
func (pg *PostgresStore) GetGroup(id uuid.UUID) (tp.Group, error) {
	query := fmt.Sprintf(`SELECT id, title FROM %s WHERE id = $1`, groupTableName)
	var g tp.Group
