...
func (pg *PostgresStore) CreateDateTrigger(dt tp.DateTrigger) (tp.DateTrigger, error) {
	query := fmt.Sprintf("INSERT INTO %s (id, scheduled_date, task_id) VALUES ($1, $2, $3)", dateTriggerTableName)
...
func (pg *PostgresStore) GetDateTrigger(id uuid.UUID) (tp.DateTrigger, error) {
	query := fmt.Sprintf("SELECT id, scheduled_date, task_id FROM %s WHERE id = $1", dateTriggerTableName)
...
func (pg *PostgresStore) ListDateTriggers() ([]tp.DateTrigger, error) {
	query := fmt.Sprintf("SELECT id, scheduled_date, task_id FROM %s", dateTriggerTableName)
...
func (pg *PostgresStore) UpdateDateTrigger(dt tp.DateTrigger) (tp.DateTrigger, error) {
	query := fmt.Sprintf("UPDATE %s SET scheduled_date = $1, task_id = $2 WHERE id = $3", dateTriggerTableName)
